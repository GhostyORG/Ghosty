...
fs.readdir("./src/servers/commands/", (err, files) => {
    ...
    console.log(`[GhostyLists/servers]: ${files.length} command loaded.`);
...
serverClient.on('ready', async () => {
    console.log("[GhostyLists/servers]: Bot successfully connected as " + serverClient.user.tag + ".");
    serverClient.user.setPresence({
        activity: {
            type: 'WATCHING',
            name: '-bump & -vote | GhostyLists/servers'
        },
        status: "dnd"
    });
});
