...
module.exports = async(req, res, next) => {
    ...
    if (!bot.auth) return res.json({ sucess: "false", error: "Create a bot authorization token."})
    if (bot.auth !== auth) return res.json({ sucess: "false", error: "Incorrect authorization token."})
