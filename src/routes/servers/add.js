...
console.log("[GhostyLists]: Add Server router loaded.");
...
app.post("/add", global.checkAuth, async (req, res) => {
    ...
    let checkGuild = await db.findOne({
        id: guildID
    });
    if (checkGuild) return res.send({
        error: true,
        message: "This server already exist system."
    });
    if (!guildID || !longDesc || !shortDesc || !tags) return res.send({
        error: true,
        message: "Fill the must any blanks."
    });
    if (!link && !autoCreate) return res.send({
        error: true,
        message: "Fill the must any blanks."
    });
    ...
    if (!member) return res.send({
        error: true,
        message: "You can only add servers with ADMINISTRATOR authorization."
    });
    if (!member.permissions.has("ADMINISTRATOR")) return res.send({
        error: true,
        message: "You can only add servers with ADMINISTRATOR authorization."
    });
    await db.updateOne({
    ...
    if (autoCreate === "true") {
        guild.fetchInvites().then(async fetchinvite => {
            fetchinvite.array().find(a => a.inviter.id === client.user.id) ?
                ...
                await guild.channels.cache.random().createInvite({
                    maxAge: 0
                });
        });
        guild.fetchInvites().then(async fetchinvite => {
    ...
    return res.send({
        success: true,
        message: "Server succesfuly added."
    });
})
